...
# 品質守門（lint.yml 在 PR 上自動跑這幾組）
python3 scripts/lint_repo.py                                     # 跨平台規則：date -v / declare -A / 硬路徑
python3 scripts/lib/accounts.py --check-workflow .github/workflows/metrics-sync.yml  # 帳號 env var 一致性
python3 scripts/validate_notion_schema.py                        # Notion 欄位名/選項污染（需 NOTION_TOKEN + DB ID）
python3 tests/make_fixtures.py && python3 scripts/analyze_by_topic.py --all --days 30 && python3 scripts/render_index.py  # smoke
```

## 新增 Threads 帳號 SOP

帳號清單唯一來源是 `scripts/lib/accounts.py`。新增帳號：
1. 在 `accounts.py:ACCOUNTS` 加一筆（key + Notion「貼文人」canonical 名，一字不差含空格）
2. 設 GitHub Secrets：`THREADS_TOKEN_<KEY>` / `THREADS_USERID_<KEY>` / `THREADS_USERNAME_<KEY>`
3. 把上述 env var 加進 `metrics-sync.yml`，`THREADS_TOKEN_<KEY>` 加進 `token-health-check.yml`
4. 開 PR — lint job 的「帳號清單一致性檢查」會驗證第 3 步沒漏（漏了會紅）
...
- Threads / IG accounts tracked: 唯一來源 `scripts/lib/accounts.py`（目前 6 個 Threads 帳號 + IG cosmate；新增帳號見「新增 Threads 帳號 SOP」）
