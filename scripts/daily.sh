...
if [ "$MODE" != "skip-scrape" ]; then
  ...
  if python3 scripts/scrape_playwright_topics.py --scroll 10; then
    echo "✅ Playwright 爬取成功（含 repost/share）"
  else
    echo "⚠️  Playwright 失敗，降級使用 Apify（無 repost/share）"
    python3 scripts/scrape_multi_topic.py anime love cosplay --timeout 300
  fi
fi
